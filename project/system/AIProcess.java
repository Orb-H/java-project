...
public class AIProcess implements Callable<int[]> {
	...
	protected int[] Stage2AI() {
		...
		if (GameSystem.mode)
			benefit = benefit + op2[0][0] - predict2;
		// benefit이 음수면 ai 가 항상 손해 보기 때문에 도망간다.
		if (benefit < 0)
			return Avoid(false, false, false);
		// 그외의 경우에는 player1에게 주는 대미지 op1[0][0]과 player2에게 주는 대미지 op2[0][0]을 반환해서 더 쎈 쪽을
		// 반환
		if (op1[0][0] > op2[0][0]) {
	...
	protected int[] Stage3AI() {
		...
		/*
		 * 플레이어가 죽지 않았으면 해당 플레이어에 대한 내가 받는 예측 대미지, 내가 줄수 있는 op 배열을 계산 한다. state3에는
		 * predictHand를 쓰지않고 실제 player의 hand를 가지고 예측을 해서 예측 정확도를 올린다.
		 */
		if (ply[0].hp > 0) {
		...
		if (benefit < -20 && ai.mp < averageCost)
			return Avoid(ai.hand[19], ai.hand[22], ai.hand[23]);
		// 양쪽에게 줄 수있는 대미지가 0이고 내 mp가 80 초과면 상대에거 접근한다.
		if (op1[0][0] == 0 && op2[0][0] == 0 && ai.mp > 80)
			return Close(ai.hand[22], ai.hand[23]);
	...
	protected int[] Avoid(boolean hasHeal, boolean hasLDash, boolean hasRDash) {
		...
		if (dist == 0) {
			if (chance < 0.125)
				return new int[] { left, 2, 5 };
			if (chance < 0.25)
				return new int[] { left, 3, 5 };
			if (chance < 0.375)
				return new int[] { right, 2, 5 };
			if (chance < 0.5)
				return new int[] { right, 3, 5 };
			if (chance < 0.625)
				return new int[] { 2, left, 5 };
			if (chance < 0.75)
				return new int[] { 2, right, 5 };
			if (chance < 0.875)
				return new int[] { 3, left, 5 };
			if (chance < 1)
				return new int[] { 3, right, 5 };
		}
		if (dist == 1) {
			if (dy == 0) {
				if (dx == -1) {
	...
	protected int[] Close(boolean hasLDash, boolean hasRDash) {
		...
		if (dy < 0 && dx > 0) {
			if (chance < 0.5)
				return new int[] { 1, 2, (ai.mp > 90) ? 4 : 5 };
			if (chance < 1)
				return new int[] { 2, 1, (ai.mp > 90) ? 4 : 5 };
		}
		if (dy >= 0 && dx > 0) {
			if (chance < 0.5)
				return new int[] { 1, 3, (ai.mp > 90) ? 4 : 5 };
			if (chance < 1)
				return new int[] { 3, 1, (ai.mp > 90) ? 4 : 5 };
		}
		if (dy >= 0 && dx <= 0) {
			if (chance < 0.5)
				return new int[] { 0, 3, (ai.mp > 90) ? 4 : 5 };
			if (chance < 1)
				return new int[] { 3, 0, (ai.mp > 90) ? 4 : 5 };
		}
		if (dy < 0 && dx <= 0) {
			if (chance < 0.5)
				return new int[] { 0, 2, (ai.mp > 90) ? 4 : 5 };
			if (chance < 1)
				return new int[] { 2, 0, (ai.mp > 90) ? 4 : 5 };
		}
	...
	protected int[][] CalcMaxDmg(int ty, int tx) {
		...
		for (int i = 1; i < 24; ++i)
			if (ai.hand[i] && Card.cards.get(i + 6).inBound(dy, dx))
				idx_card[r++] = i + 6;
		// ty,tx 에 닿는 공격 카드가 ai의 패에 없고 mp가 100이고 star 카드가 있으면 star를 사용, 그 외에는 random
		// move
		if (r == 0) {
